...
import {useMaterialCategory} from '../../../hooks/MRP/Master/useMaterialCategory';

export default function MaterialCategory(){
  const { MaterialCategoryData, loading, data, error,MaterialCategorySave,MaterialCategoryUpdate }=useMaterialCategory();
  ...
  // ----------------------FORMIK SUBMIT /EDIT DATA ----------------------
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try{
        if(isEditing){
          await MaterialCategoryUpdate(values.id,values);
        }else{
          await MaterialCategorySave(values);
        }
        await MaterialCategoryData();
        resetForm();
        setIsEditing(false);
      }
      catch(err){
